# Custom data loader for face images in every frame and audio 
...
class CustomDataset(Dataset):
    def __init__(self,audio_dataset_path,images_dataset_path,data_augmentation=False,musan_path=None):
        ...
        with open('video_vox1_data_person_id.pkl', 'rb') as f:
            self.data_person_id = pickle.load(f)
        # Load dictionary from file
        with open('video_vox1_sample_id_to_path.pkl', 'rb') as f:
            self.sample_id_to_path = pickle.load(f)
            # Load dictionary from file
        with open('video_vox1_sample_id_to_person.pkl', 'rb') as f:
    ...
    def __getitem__(self, index):
        ...
        try:
            # just fix a bug in data
            if(path[-len("00010.jpg"):] == "00010.jpg"):
                wav_input, sample_rate = torchaudio.load(os.path.join(self.audio_dataset_path,path[:-len("00010.wav")] + "000010.wav")) #load a tensor
            else:
                wav_input, sample_rate = torchaudio.load(os.path.join(self.audio_dataset_path,f"{path[:-4]}.wav")) #load a tensor
            ...
            print("chyba")
            return [torch.zeros(48_000).to(self.worker_id),torch.zeros(75,3,112,112).to(self.worker_id)], torch.zeros(self.persons).to(self.worker_id)
        ...
        #video added
        individual_images = torch.split(image, 112, dim=2)
        image = torch.stack(individual_images, dim=0)
        #video added
        ...
        if (self.data_augmentation == True):
            if(torch.rand(1) < 0.8): # augment in 80%
                ...
                if(torch.rand(1) < 0.5):
                    ...
                    subsequence = self.rand_transform(subsequence)
